...
- **255 之后的 rc=3 会先复核，不直接判 deferred**：如果本次 run 里此前已经出现过一次 255（SSH 传输层失败），那次失败可能发生在远端"已经部署成功"之后——即 `compose up` + 探针通过 + `last_good_tag` 已经写成本次 SHA，只是退出码没能传回 runner。这种情况下重试拿到的 rc=3（忙锁被新容器占住）不能证明"旧容器还在跑"，只能证明"此刻拿不到锁"。所以 workflow 会先 SSH 读一次远端 `last_good_tag`：如果它已经等于本次 `GIT_SHA`，说明前一次尝试其实已经成功，直接按成功收尾（不发 deferred 黄卡）；否则维持原有 deferred 语义（黄卡 + 不 @全员），但日志里会额外标注"状态不确定，建议核实"，提示这次的 deferred 判断是在传输不稳的背景下做出的。
