...
- **255 之后的 rc=3 会先复核，不直接判 deferred**：如果本次 run 里此前已经出现过一次 255（SSH 传输层失败），那次失败可能发生在远端"已经部署成功"之后——即 `compose up` + 探针通过 + `last_good_tag` 已经写成本次 SHA，只是退出码没能传回 runner。这种情况下重试拿到的 rc=3（忙锁被新容器占住）不能证明"旧容器还在跑"，只能证明"此刻拿不到锁"。所以 workflow 会在门禁开启时先 SSH 读一次远端 `last_good_tag` 作为 run 开始前的基线，rc=3 复核时再读一次当前值——只有当前值等于本次 `GIT_SHA`、**且**它与 run 前的基线不同（即确实是本次 run 期间才变成这个值）、**且**基线本身取到了（不是 ssh 失败导致的未知），三者同时成立才判定"前一次尝试其实已经成功"，直接按成功收尾（不发 deferred 黄卡）。这是为了堵住一种假阳性：手动重跑一个已经部署过的 SHA，或两次 push 恰好产生同一个 `GIT_SHA` 时，远端 `last_good_tag` 在这次 run 开始前就已经等于本次 SHA 了，是历史遗留值而非本次新写——光比对现值会把这种情况误判为成功，掩盖一次可能从未真正通过健康检查的部署。三条中任一条不满足，都维持原有 deferred 语义（黄卡 + 不 @全员），日志里额外标注"状态不确定，建议核实"，提示这次的 deferred 判断是在传输不稳的背景下做出的。
